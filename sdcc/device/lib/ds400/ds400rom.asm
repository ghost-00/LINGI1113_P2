...
;------------------------------------------------------------
;------------------------------------------------------------
;	ds400rom.c:244: void _romcall(void) __naked
;	-----------------------------------------
;	 function _romcall
;	-----------------------------------------
__romcall:
;	naked function: no prologue.
;	ds400rom.c:289: __endasm ;
      ...
      ; point to the address of the table
	      ...
	      inc dptr
	      clr a movc a, @a+dptr ; get the address of the table
	      push acc
	      inc dptr
	      clr a movc a, @a+dptr
...
;------------------------------------------------------------
;------------------------------------------------------------
;	ds400rom.c:293: void _romredirect(void) __naked
;	-----------------------------------------
;	 function _romredirect
;	-----------------------------------------
__romredirect:
;	naked function: no prologue.
;	ds400rom.c:326: __endasm;	
...
;------------------------------------------------------------
;	ds400rom.c:346: unsigned char init_rom(void __xdata *loMem,
;	-----------------------------------------
;	 function init_rom
;	-----------------------------------------
_init_rom:
;	naked function: no prologue.
;	ds400rom.c:370: __endasm	;
 ...
 ; hiMem is in _init_rom_PARM_2
	 ...
	 mov 0x1E, #((69 * 3) & 0xff) mov 0x1F, #(((69 * 3) >> 8) & 0xff) lcall __romcall
...
;------------------------------------------------------------
;------------------------------------------------------------
;	ds400rom.c:375: unsigned long task_gettimemillis_long(void) __naked
;	-----------------------------------------
;	 function task_gettimemillis_long
;	-----------------------------------------
_task_gettimemillis_long:
;	naked function: no prologue.
;	ds400rom.c:386: __endasm;
	
    ; no parameters to load.
	    mov 0x1E, #(( 8 * 3) & 0xff) mov 0x1F, #((( 8 * 3) >> 8) & 0xff) lcall __romredirect
...
;------------------------------------------------------------
;------------------------------------------------------------
;	ds400rom.c:389: unsigned char task_getthreadID(void) __naked
;	-----------------------------------------
;	 function task_getthreadID
;	-----------------------------------------
_task_getthreadID:
;	naked function: no prologue.
;	ds400rom.c:397: __endasm;    
	
    ; no parameters to load.
	    mov 0x1E, #(( 9 * 3) & 0xff) mov 0x1F, #((( 9 * 3) >> 8) & 0xff) lcall __romredirect
...
;------------------------------------------------------------
;------------------------------------------------------------
;	ds400rom.c:400: unsigned int task_gettickreload(void)
;	-----------------------------------------
...
_task_gettickreload:
	...
	ar0 = 0x00
	ar1 = 0x01
;	ds400rom.c:402: return DSS_timerReload;
...
;------------------------------------------------------------
;	ds400rom.c:405: void task_settickreload(unsigned int rl)
;	-----------------------------------------
...
;	ds400rom.c:407: DSS_timerReload = rl;
00101$:
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
